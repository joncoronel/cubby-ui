...
Builds a collection for the `items` prop from flat or grouped data. Each item's selection value comes from `getValue`, which must return a primitive (usually a stable ID), and its display and filter label from `getLabel`. Items can't have their own `items` array field, since that marks a group.
