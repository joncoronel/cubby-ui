...
// 3. Render
<Command {...rootProps} filter={null}>
  <CommandVirtualizedList scrollRef={scrollRef} totalSize={totalSize}>
...
function useCommandFilter(): {
  contains: (item: T, query: string, getItemLabel: (item: T) => string) => boolean;
  startsWith: (item: T, query: string, getItemLabel: (item: T) => string) => boolean;
  endsWith: (item: T, query: string, getItemLabel: (item: T) => string) => boolean;
}
```
