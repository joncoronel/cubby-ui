...
Same effect as `solidSurface`, but paints the rim on an `::after` pseudo-element at `z-[2]` instead of on the popup's own box-shadow. Use this whenever sticky or opaque children sit near the surface's edges. With `solidSurface` the rim lives in the box-shadow, so any opaque child at an edge renders on top of it and hides it. Two common cases: **sticky group labels** (Select, Combobox, Autocomplete) and **opaque inset footers** (Dialog, Alert Dialog, Sheet, Drawer, where the `inset` footer's `bg-muted` would otherwise cover the bottom rim).
...
The edge ring matches `solidSurface` exactly in both themes; only the drop is rotated to point at the content. Pass the content-facing edge, or derive it from the attach side with `INNER_EDGE_FROM_ATTACH_SIDE` (`right → left`, `bottom → top`, etc.).
