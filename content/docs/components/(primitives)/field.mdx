...
`FieldControl` inherits its visual styling from the [Input](/docs/components/input) component (`inputVariants`). Changes to Input's variant styles propagate to every `FieldControl` — keep that in mind when customizing either.
