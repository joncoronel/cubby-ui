...
The same layering applies to border style and width: classes like
`border-dashed` or `border-2` on the root style a border you cannot see.
Target the layer instead, for example
`*:data-[slot=button-background]:border-dashed`.
