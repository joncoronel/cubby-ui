...
| `shadowLevel` | Drop layers         | Feel                                     |
| ------------- | ------------------- | ---------------------------------------- |
| `1`           | 0 (just a 1px ring) | Barely there                             |
| `2`           | 1                   | Tooltip, quiet                           |
| `3`           | 2                   | **Dropdown, popover, card**              |
| `4`           | 3                   | Toast, floating elements with no trigger |
| `5`           | 4                   | **Dialog, sheet**                        |
| `6`           | 5                   | Transitional                             |
| `7`           | 6                   | Dramatic (hero modals)                   |
| `8`           | 7                   | Maximum gravity (rare)                   |
...
It is not an inverse surface. Inverting in dark would put a white slab at roughly 97 times the page's luminance into chrome you cannot scroll away from, so both themes stay dark. In light the fill borrows `--neutral`, the same black as a solid `neutral` Button. In dark it steps _below_ the page rather than above it, because up is where every card lives and a lifted near-black slab reads as one more card.

Put `data-surface="chrome"` on the container and use ordinary page classes inside. That attribute re-points `--foreground`, `--muted-foreground`, `--border` and the hover and active washes for the whole subtree, so anything you drop in paints correctly with no per-control classes:

```tsx
import { CHROME_SURFACE } from "@/registry/default/lib/elevated";

<div data-surface="chrome" className={`rounded-2xl ${CHROME_SURFACE}`}>
  <span className="text-foreground">Primary label</span>
  <span className="text-muted-foreground">Secondary label</span>
  <Button variant="ghost">Ghost buttons work too</Button>
</div>;
```
...
<div
  data-surface="chrome"
  className={cn(CHROME_SURFACE, "shadow-(--chrome-shadow)")}
/>
```
...
- **The edge is not the ladder's.** `--surface-rim-*` and the ring baked into `--surface-shadow-*` both assume a fill lighter than the page. This fill is darker, so the ladder's ring lands lighter than the very fill it should define and the boundary blurs. `--chrome-hairline` (outer boundary) and `--chrome-rim` (top-edge specular) replace it. Both are box-shadow values, not colors: writing a bare color into either one voids the entire `box-shadow` it lands in, because a single invalid layer invalidates the whole declaration.
...
--surface-shadow-1 .. -8         /* drop-shadow recipes, read by the helpers */
--surface-rim-1 .. -8            /* inset rim recipes, read by the helpers */
--surface-shadow-combined-1 .. -8 /* drop + rim in one value, for solidSurface */

/* State */
--surface-hover                  /* hover overlay */
--surface-active                 /* pressed overlay */
--surface-selected               /* selected overlay */
...
/* Chrome surface (off the ladder) */
--chrome                         /* near-black fill in both themes → bg-chrome */
--chrome-foreground              /* label color on that fill */
--chrome-muted-foreground        /* secondary label color */
--chrome-hover                   /* the one wash: hover, active and fill */
--chrome-border                  /* hairline between controls */
--chrome-shadow                  /* the opt-in edge (a box-shadow value) */
