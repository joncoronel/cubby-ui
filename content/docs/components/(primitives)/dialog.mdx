...
<ComponentPreview component="dialog" example="dialog-persistent-close" />
...
The close button sits at the top of the card, so under `scroll="outside"` it scrolls out of view on a tall dialog. Escape still closes it. Give a long dialog a footer close action, or see [Persistent Close Button](#persistent-close-button) for one that stays on screen.
