...
<ApiProp
  ...
  defaultValue="true"
>
  Use native browser scrolling instead of the custom ScrollArea component. Defaults to `true` for drawer because ScrollArea can cause animation stutter with the drawer's scroll-snap mechanics. Set to `false` to use ScrollArea features like fade edges and custom scrollbars.
</ApiProp>
