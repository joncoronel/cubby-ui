...
On a Card, Dialog, or popover surface, the opaque `default` input collapses into its parent. Use `variant="elevated"` on `ComboboxInput` for a translucent overlay that adapts to the substrate. See the [Surfaces](/docs/getting-started/surfaces) docs.
