...
### Text Roll
Any label that swaps in place rolls glyph by glyph with Scritto (`MorphText` in `components/docs/morph-text.tsx`): it doesn't animate on mount, animates each change after, on Scritto's own spring (400ms here, 1.5% overshoot) with the box width easing separately so neighbours never overshoot, and steps straight to the new text under reduced motion. Labels that answer a click ("Copied", "Hide code") use `feedback`, 209ms. Used for the shelf trigger's group and page name, the header section crumb, the mobile pill's section, Copy page, and the Code toggle. `@scritto/core/ssr.css` (imported in globals.css) keeps widths identical through the element's upgrade. Headings are flattened to plain text first. Responsive visibility classes go on a wrapper, because the element sets its own display; long labels use `MorphText truncate`, which clips on a wrapper and fades the right edge only when the settled text doesn't fit (the fit is re-checked just after each roll ends, since mid-roll the outgoing glyphs still take up room). Those scroll-driven labels also size themselves: a hidden copy of the text measures the new width and the wrapper transitions to it in CSS (Scritto's box curve, `cubic-bezier(0.22, 1, 0.36, 1)`), because Scritto doesn't retarget a box-width animation already in flight, and on a fast scroll its box eased toward a stale width behind the text. Inside that wrapper the Scritto element keeps `min-width: max-content`: Scritto switches to wrapping (with `!important` in its shadow root) whenever its box is narrower than the value, which the wrapper is while it catches up to a longer label. A scroll-driven label that changes again mid-roll starts on a fresh Scritto element (mounted on the previous label, rolling to the new one a frame later), since Scritto also doesn't retarget an in-flight glyph offset and the final label otherwise sat shifted, then slid into place. On first load, each server-rendered element is seeded with its own text the moment it's defined: Scritto 0.1.0 doesn't read the text in the tag, and the label was blank until hydration.
...
Anything visible on first paint must not change when React hydrates. Platform shortcut hints render both variants and CSS picks one from `<html data-platform>`, set by an inline script in the head; the theme toggle renders both icons and `.dark` picks one; Text rolls use Scritto, whose element upgrade does only local work; the earlier torph version split and measured every label at load, which (with ~150 `:has()` selectors on the page) forced dozens of whole-page style recalcs during hydration. Styles for portaled popups use literal values, not `--docs-*` tokens, which only exist inside `.docs-root`.
