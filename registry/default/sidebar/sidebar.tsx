    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delay={0}>
        <div
          ...
          className={cn(
            "group/sidebar-wrapper has-[[data-slot=sidebar][data-variant=inset]]:bg-muted flex min-h-svh w-full has-[[data-slot=sidebar][data-variant=inset]]:h-svh has-[[data-slot=sidebar][data-variant=inset]]:overflow-hidden",
            className,
          )}
          {...props}
        >
  ...
  return (
    ...
    >
      <div
        ...
        className={cn(
          "relative w-(--sidebar-width) bg-transparent transition-[width] duration-150 ease-[cubic-bezier(0.65,0,0.35,1)]",
      ...
      <div
        ...
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-150 ease-[cubic-bezier(0.65,0,0.35,1)] md:flex",
          side === "left"
      ...
      >
        <div
          ...
          className="bg-muted group-data-[variant=floating]:border-border/60 flex h-full w-full flex-col overflow-visible group-data-[variant=floating]:rounded-2xl group-data-[variant=floating]:border group-data-[variant=floating]:shadow-[0_2px_6px_0_oklch(0.18_0_0_/_0.05)]"
        >
          {children}
        </div>
      </div>
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "bg-card relative flex w-full min-w-0 flex-1 flex-col transition-[margin,border-radius,box-shadow] duration-150 ease-[cubic-bezier(0.65,0,0.35,1)]",
        "md:peer-data-[variant=inset]:bg-background md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:overflow-auto md:peer-data-[variant=inset]:rounded-2xl md:peer-data-[variant=inset]:border md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2",
...
function SidebarInput({
  ...
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "bg-background focus-visible:outline-ring h-8 w-full shadow-none",
        className,
      )}
      {...props}
    />
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      style={{
        scrollbarColor:
          "color-mix(in oklab, var(--color-border) 50%, transparent) transparent",
      }}
      {...props}
    />
  );
...
function SidebarGroupLabel({
  ...
  const defaultProps = {
    className: cn(
      "text-muted-foreground flex h-8 shrink-0 items-center rounded-md px-2.5 text-xs font-semibold uppercase tracking-wide outline-hidden transition-[margin,opacity] duration-200 ease-linear [&>svg]:size-4 [&>svg]:shrink-0 focus-visible:outline-ring/30 outline-0 outline-offset-0 outline-transparent transition-[outline-width,outline-offset,outline-color] duration-100 ease-out outline-solid focus-visible:outline-2 focus-visible:outline-offset-2",
  ...
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 pl-3.5 text-left text-sm outline-hidden transition-[colors,background-color] duration-100 ease-out hover:bg-(--surface-hover) hover:text-accent-foreground hover:duration-0 active:bg-(--surface-selected) active:text-accent-foreground disabled:pointer-events-none disabled:opacity-60 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-60 data-[active=true]:bg-primary/10 data-[active=true]:text-primary data-[active=true]:font-medium data-[state=open]:bg-(--surface-selected) data-[state=open]:text-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 select-none focus-visible:outline-ring/30 outline-0 outline-offset-0 outline-transparent transition-[outline-width,outline-offset,outline-color] duration-100 ease-out outline-solid focus-visible:outline-2 focus-visible:outline-offset-2",
  {
    variants: {
      ...
      size: {
        ...
        lg: "h-10 text-sm px-3 py-2 group-data-[collapsible=icon]:p-0!",
      },
    },
    defaultVariants: {
      variant: "default",
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "peer-hover/menu-button:text-accent-foreground peer-data-[active=true]/menu-button:text-primary",
        "peer-data-[size=sm]/menu-button:top-1",
        "peer-data-[size=default]/menu-button:top-1.5",
        "peer-data-[size=lg]/menu-button:top-2.5",
        "group-data-[collapsible=icon]:hidden",
...
function SidebarMenuSubButton({
  ...
  const defaultProps = {
    className: cn(
      "text-foreground hover:bg-(--surface-hover) hover:text-accent-foreground hover:duration-0 active:bg-(--surface-selected) active:text-accent-foreground [&>svg]:text-muted-foreground flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2.5 py-1.5 outline-hidden transition-[colors,background-color] duration-100 ease-out select-none [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 disabled:pointer-events-none disabled:opacity-60 aria-disabled:pointer-events-none aria-disabled:opacity-60 focus-visible:outline-ring/30 outline-0 outline-offset-0 outline-transparent transition-[outline-width,outline-offset,outline-color] duration-100 ease-out outline-solid focus-visible:outline-2 focus-visible:outline-offset-2",
